...
This skill uses the GitHub CLI (`gh`) to find repositories, list and search issues and pull requests, read PR comments, match PRs by branch name, create and edit issues and pull requests, comment on issues and PRs, and retrieve other GitHub information. It interprets the user's intent, determines the correct repository context, constructs the appropriate `gh` commands, and presents results clearly.
...
| Action | gh Command(s) |
|---|---|
| `find-repos` | `gh repo list [owner]` or `gh search repos <terms>` |
| `list-issues` | `gh issue list --repo <repo> [filters]` |
| `view-issue` | `gh issue view <number> --repo <repo>` |
| `list-prs` | `gh pr list --repo <repo> [filters]` |
| `view-pr` | `gh pr view <number-or-branch> --repo <repo>` |
| `pr-comments` | `gh pr view <number> --repo <repo> --comments` |
| `find-pr-by-branch` | `gh pr list --repo <repo> --head <branch>` |
| `create-issue` | `gh issue create --repo <repo> --title <title> [--body <body>] [--label <label>] [--assignee <user>]` |
| `edit-issue` | `gh issue edit <number> --repo <repo> [--title <title>] [--body <body>] [--add-label <label>] [--remove-label <label>]` |
| `close-issue` | `gh issue close <number> --repo <repo> [--reason <completed\|not_planned>]` |
| `reopen-issue` | `gh issue reopen <number> --repo <repo>` |
| `comment-issue` | `gh issue comment <number> --repo <repo> --body <body>` |
| `create-pr` | `gh pr create --repo <repo> --title <title> [--body <body>] [--base <branch>] [--head <branch>] [--draft]` |
| `edit-pr` | `gh pr edit <number> --repo <repo> [--title <title>] [--body <body>] [--add-label <label>] [--add-reviewer <user>]` |
| `close-pr` | `gh pr close <number> --repo <repo>` |
| `merge-pr` | `gh pr merge <number> --repo <repo> [--merge\|--squash\|--rebase] [--delete-branch]` |
| `comment-pr` | `gh pr comment <number> --repo <repo> --body <body>` |
| `review-pr` | `gh pr review <number> --repo <repo> [--approve\|--request-changes\|--comment] [--body <body>]` |
| `search` | `gh search issues`, `gh search prs`, `gh search repos` |
...
- You SHOULD also check `gh repo view --json nameWithOwner -q .nameWithOwner` as a fallback
- You MUST use `--repo <owner/repo>` flag explicitly in all `gh` commands rather than relying on implicit detection, unless the user is clearly asking about repos in general
...
Confirm that `gh` is authenticated before running any GitHub query or mutation.

**Constraints:**
- You MUST run `gh auth status` before proceeding with any `gh` command that requires GitHub access
- If `gh auth status` fails or reports no logged-in account, you MUST stop and instruct the user to authenticate first
- You MUST recommend PAT-based login for non-interactive agent workflows with: `printf '%s' "$GITHUB_PAT" | gh auth login --with-token`
- You MUST NOT continue to repo detection, reads, or writes against GitHub until authentication succeeds
- You SHOULD treat existing `gh` login as sufficient; you do not need to distinguish whether the stored token originally came from browser login or direct PAT input

### 2. Determine Action and Build Command

Map the user's query to the correct `gh` command and flags.
...
- "show PR 42" or "PR #42" → `gh pr view 42 --repo <repo>`
- "comments on PR 42" → `gh pr view 42 --repo <repo> --comments`
- "PR for branch feature/foo" → `gh pr list --repo <repo> --head feature/foo`
- "my open issues" → `gh issue list --repo <repo> --assignee @me`
- "find repos about neovim" → `gh search repos neovim`
- "closed PRs by alice" → `gh pr list --repo <repo> --state closed --author alice`
- "issues with label bug" → `gh issue list --repo <repo> --label bug`
- "create an issue about login bug" → `gh issue create --repo <repo> --title "Login bug" --body "<details>"`
- "close issue 15" → `gh issue close 15 --repo <repo>`
- "comment on issue 8 saying it's fixed" → `gh issue comment 8 --repo <repo> --body "It's fixed"`
- "create a PR for this branch" → `gh pr create --repo <repo> --title "<title>" --body "<body>"`
- "create a draft PR" → `gh pr create --repo <repo> --title "<title>" --draft`
- "merge PR 50 with squash" → `gh pr merge 50 --repo <repo> --squash --delete-branch`
- "approve PR 33" → `gh pr review 33 --repo <repo> --approve`
- "request changes on PR 33" → `gh pr review 33 --repo <repo> --request-changes --body "<feedback>"`
- "add label bug to issue 12" → `gh issue edit 12 --repo <repo> --add-label bug`
...
**Constraints:**
- You MUST run the constructed `gh` command using Bash
- You MUST check for errors: if `gh` is not authenticated, instruct the user to run `printf '%s' "$GITHUB_PAT" | gh auth login --with-token`
...
- You MUST NOT fabricate or summarize data that was not returned by `gh` — only present what the CLI returned
...
- You MUST offer relevant follow-up options based on what was shown:
  ...
  - After viewing a PR: offer to show comments, approve/review, merge, open in browser (`gh pr view --web`), or check out the branch (`gh pr checkout <number>`)
...
gh pr list --repo owner/repo --head feature/dark-mode
```
...
gh search repos neovim lua plugins --limit 20
```

### List repos for an org
```
User: list repos in the acme org
gh repo list acme --limit 30
```

### Closed PRs by a specific author
```
User: show closed PRs by alice
gh pr list --repo owner/repo --state closed --author alice
```

### Create an issue
```
User: create an issue about the broken dark mode toggle
gh issue create --repo owner/repo --title "Broken dark mode toggle" --body "The dark mode toggle on the settings page does not persist the preference."
```
...
gh issue comment 15 --repo owner/repo --body "Duplicate of #10"
```

### Create a pull request
```
User: create a PR for this branch
gh pr create --repo owner/repo --title "Add dark mode support" --body "Implements dark mode toggle and persistence." --head feature/dark-mode
```

### Create a draft PR
```
User: create a draft PR
gh pr create --repo owner/repo --title "WIP: Refactor auth module" --draft
```

### Merge a PR
```
User: squash merge PR 50
gh pr merge 50 --repo owner/repo --squash --delete-branch
```

### Approve a PR
```
User: approve PR 33
gh pr review 33 --repo owner/repo --approve
```

### Request changes on a PR
```
User: request changes on PR 33, the tests are missing
gh pr review 33 --repo owner/repo --request-changes --body "Please add tests for the new endpoints."
```
...
### Not authenticated
If `gh` returns an auth error, or `gh auth status` reports no authenticated account:
- Instruct the user to run `printf '%s' "$GITHUB_PAT" | gh auth login --with-token`
...
- Suggest `gh repo list <owner>` to verify available repos
...
- The branch name may differ — suggest `gh pr list` without the head filter to browse
